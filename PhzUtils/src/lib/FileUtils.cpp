...
void createDirectoryIfAny(const std::string& directory_name) {
  ...
  if (!directory_name.empty()) {
    ...
    if (!fs::exists(fs_dir)) {
      ...
      throw Elements::Exception() << "The path :  " << directory_name << " is not a directory!"
...
void checkWritePermission(const std::string& directory_name_and_or_filename, bool path_only) {
  ...
  if ((test_fstream.rdstate() & std::fstream::failbit) != 0) {
    throw Elements::Exception() << " IO error, can not write any file there : %s " << directory_name_and_or_filename
                                << " (from the CheckDirWritePermission function) ";
  }
  test_fstream.close();
  // Remove file created
  if (std::remove(filename.c_str())) {
    logger.warn() << "Removing temporary file creation failed: \"" << filename << "\" (from CheckDirWritePermission)!";
  }
}
