...
BOOST_AUTO_TEST_CASE(constructorWithOverlappingGroups) {
  ...
  // Given
  QualifiedNameGroupManager::group_list_type groups{{"first", {{"one"}, {"two"}, {"common"}, {"three"}}},
...
BOOST_AUTO_TEST_CASE(getManagedGroups) {
  ...
  // Given
  QualifiedNameGroupManager::group_list_type groups{{"first", {{"one"}, {"two"}, {"three"}}},
  ...
  for (auto& pair : result) {
    ...
    BOOST_CHECK_EQUAL_COLLECTIONS(pair.second.begin(), pair.second.end(), groups.at(pair.first).begin(),
                                  groups.at(pair.first).end());
  }
}
...
BOOST_AUTO_TEST_CASE(findGroupContaining) {
  ...
  for (auto& group : groups) {
    for (auto& name : group.second) {
      ...
      BOOST_CHECK_EQUAL_COLLECTIONS(result.second.begin(), result.second.end(), group.second.begin(),
                                    group.second.end());
    }
  }
