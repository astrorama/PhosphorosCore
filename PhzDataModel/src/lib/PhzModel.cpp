...
ModelAxesTuple createAxesTuple(std::vector<double> zs, std::vector<double> ebvs,
  ...
  GridContainer::GridAxis<XYDataset::QualifiedName> reddening_curves_axis{"Reddening Curve",
                                                                          std::move(reddening_curves)};
  GridContainer::GridAxis<XYDataset::QualifiedName> sed_axis{"SED", std::move(seds)};
  return ModelAxesTuple{std::move(z_axis), std::move(ebv_axis), std::move(reddening_curves_axis), std::move(sed_axis)};
}
