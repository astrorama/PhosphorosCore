...
void writeAdjustErrorParamMap(std::ostream& out, const AdjustErrorParamMap& adjustErrorParam) {
  ...
  for (auto& param : adjustErrorParam) {
    std::vector<Euclid::Table::Row::cell_type> param_data {
      param.first.qualifiedName(), std::get<0>(param.second), std::get<1>(param.second), std::get<2>(param.second) };
...
AdjustErrorParamMap readAdjustErrorParamMap(std::istream& in) {
  ...
  if (column_info->find("Filter") == nullptr
      || column_info->find("Alpha") == nullptr
      || column_info->find("Beta") == nullptr
      || column_info->find("Gamma") == nullptr) {
    throw Elements::Exception() << "readPhotometricCorrectionMap: table must contain columns 'Filter', 'Alpha', 'Beta' and 'Gamma'";
  }
  ...
  for (auto& row : table) {
    ...
    map.emplace(std::make_pair(std::move(filter_name),
        std::make_tuple(boost::get<double>(row["Alpha"]),
                        boost::get<double>(row["Beta"]),
