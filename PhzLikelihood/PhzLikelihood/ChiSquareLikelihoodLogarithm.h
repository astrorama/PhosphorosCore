...
public:
  ...
  double operator()(SourceIter source, SourceIter source_end, ModelIter model, double scale) const {
    double sum{0.0};
...
#pragma GCC diagnostic ignored "-Wfloat-equal"
    double error_square = (source.error != 0) ? (source.error * source.error) : std::numeric_limits<double>::min();
...
public:
  ...
  double operator()(const SourceCatalog::FluxErrorPair& source, const SourceCatalog::FluxErrorPair& model,
    ...
    if (source.upper_limit_flag) {
      double err_func = std::erf((source.flux - scale * model.flux) / (Elements::Units::sqrt_of_two * source.error));
      if (err_func <= -1.) {
...
/// for missing data
using ChiSquareLikelihoodLogarithmMissingData =
    _Impl::ChiSquareLikelihoodLogarithm_Impl<_Impl::ChiSquareMissingData<_Impl::ChiSquareNormal>>;
...
using ChiSquareLikelihoodLogarithmUpperLimitMissingData =
    _Impl::ChiSquareLikelihoodLogarithm_Impl<_Impl::ChiSquareMissingData<_Impl::ChiSquareUpperLimit>>;
