...
public:
  ...
  MOCK_METHOD3(FunctorCall, double(const SourceCatalog::FluxErrorPair& source,
                                   const SourceCatalog::FluxErrorPair& model, double scale));
};
std::unique_ptr<MockChiSquareFunc> mock_chi_square_func_singleton{};

class TestChiSquareFunc {
public:
  double operator()(const SourceCatalog::FluxErrorPair& source, const SourceCatalog::FluxErrorPair& model,
                    double scale) {
...
struct ChiSquareLikelihoodLogarithm_Fixture {
  ...
  vector<double> chi_square_data{
      1063.785869015169,      172.0599603523264,      0.8992329001838121,   54.95736642114042,
      0.2599919082512001,     0.8555984676950313,     1.1685450121842822,   0.0054692038213795076,
      0.0469884039509647,     20.77532734336956,      0.009177340233459896, 73.44661276127589,
      582.6032489007681,      0.0033762997891703755,  1005.0200968362471,   0.11466039252900344,
      1.1623349268493552,     0.025664629164904248,   15.68102696291849,    5.469077989174785,
      3.9484818861560735,     708.5959821406087,      0.09476159959309376,  1.8354663976251606e-05,
      5.785715067997423,      8.70220406834054,       0.7219051506838788,   0.6739487908941751,
      0.49651625665536187,    4.988272772285099,      7.667125669023741,    10.445837628980895,
      3.5616886373137904,     0.38490304380797014,    0.005987815552239399, 0.6119848833046667,
      0.1916829746715228,     1.936174542520018,      0.12077362832664866,  794.8565413079425,
      3300.3592777680283,     1.2206214496630676,     5.480080907694947,    7.546135418622665,
      1.1671776949671053,     0.028322085035574287,   0.05459980956624694,  105.90630070766044,
      0.1214139811260656,     24.90714544566146,      65.856364381918,      21.61491977836802,
      0.041008859912110465,   0.11618857000247743,    80.47773686095353,    0.05264927987935968,
      0.1147079043724327,     17.112145606482173,     0.7035340245931965,   0.3426809704996635,
      132.52801640264903,     0.029001607248698606,   0.025738043339410224, 2.2813501394205424,
      0.00038472186196538023, 0.14056472132149544,    13.043119580373402,   1.565911942084213,
      14.513370954265085,     0.9599895710311656,     0.059695222435737945, 0.005124782797586656,
      1.1415753588689546,     5.6314898530910093e-05, 61.860463421328646,   0.0021897559878807653,
      87.04182521483295,      1.163641557237057,      5.057486573995309,    3.7917900873351718,
      0.018600788811751322,   0.971397050484918,      0.036544065516784234, 0.2144156286151374,
      0.09079772071347145,    2.4239204486058705,     2026.228650260469,    13.443509384484003,
      0.5836961012507009,     1.992842085440592,      18.897163193856198,   13.522337774153192,
      0.03841883373576135,    2.333930292692205,      0.28602649991146584,  0.38644164119030805,
      10.996752239529465,     0.9575193449347575,     0.21520580281696183,  3.1198270113748054};

  vector<double> upper_limit_data{-0.0,
                                  1416.7928370645282,
                                  0.37626177515910625,
                                  60.83746752577336,
                                  2.374474870197735,
  ...
  vector<double> upper_limit_fast_data{0.,
                                       1416.7928370645282,
                                       0.,
                                       1416.7928370645282,
                                       1416.7928370645282,
...
BOOST_AUTO_TEST_CASE(ChiSquareLikelihoodLogarithm_Impl_test) {
  ...
  for (auto s_it = source_values.begin(), m_it = model_values.begin(); s_it != source_values.end(); ++s_it, ++m_it) {
    EXPECT_CALL(*mock_residual_calculator_singleton, FunctorCall(*s_it, *m_it, scale)).WillOnce(Return(1));
  }

  // When
  double sum = chi_square_functor(source_values.begin(), source_values.end(), model_values.begin(), scale);
...
BOOST_AUTO_TEST_CASE(ChiSquareMissingData_test) {
  ...
  EXPECT_CALL(*mock_chi_square_func_singleton, FunctorCall(sources[0], models[0], scales[0]))
      .WillOnce(Return(expected[0]));
  EXPECT_CALL(*mock_chi_square_func_singleton, FunctorCall(sources[1], models[1], scales[1]))
      .WillOnce(Return(expected[1]));
  EXPECT_CALL(*mock_chi_square_func_singleton, FunctorCall(sources[3], models[3], scales[3]))
