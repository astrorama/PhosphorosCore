...
public:
  std::shared_ptr<std::vector<std::string>> source_filters{
      new std::vector<std::string>{"Filter1", "Filter2", "Filter3"}};
  std::vector<SourceCatalog::FluxErrorPair> source_fluxes{{1., 1.}, {2., 2.}, {3., 3.}};
...
private:
  std::vector<SourceCatalog::Photometry>
  makeModelPhotList(int arg_model_no, std::shared_ptr<std::vector<std::string>> arg_model_filters) {
...
BOOST_FIXTURE_TEST_CASE(SameFilterOrder, LikelihoodAlgorithmFixture) {
  ...
  //            };
  PhzLikelihood::LikelihoodLogarithmAlgorithm likelihood_algo{scale_factor_calc_mock.getFunctorObject(),
...
BOOST_FIXTURE_TEST_CASE(MissingSourcePhotometry, LikelihoodAlgorithmFixture) {
  ...
  PhzLikelihood::LikelihoodLogarithmAlgorithm likelihood_algo{scale_factor_calc_mock.getFunctorObject(),
                                                              likelihood_calc_mock.getFunctorObject()};
  likelihood_algo(source_phot_missing, model_phot_list.begin(), model_phot_list.end(), likelihood_list.begin(),
                  scale_factor_list.begin());
