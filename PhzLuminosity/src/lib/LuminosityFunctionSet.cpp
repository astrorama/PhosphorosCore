...
LuminosityFunctionSet::LuminosityFunctionSet(
    std::vector<std::pair<LuminosityFunctionValidityDomain, std::unique_ptr<MathUtils::Function>>> luminosityFunctions)
    ...
    if (processed.find(group) == processed.end()) {
      ...
      for (auto& pair_z_search : m_luminosity_functions) {
        if (pair_z_search.first.getSedGroupName() == group) {
          z_ranges.push_back(std::make_pair(pair_z_search.first.getMinZ(), pair_z_search.first.getMaxZ()));
        }
      }
        ...
        for (size_t index_2 = index_1 + 1; index_2 < z_ranges.size(); ++index_2) {
          if (z_ranges[index_1].first < z_ranges[index_2].second) {
            if (z_ranges[index_1].second > z_ranges[index_2].first) {
              throw Elements::Exception() << "In the LuminosityFunctionSet ranges must not overlap";
            }
          }
...
LuminosityFunctionSet::getLuminosityFunction(const std::string& sedGroup, double z) const {
  ...
  throw Elements::Exception() << "No Luminosity Function is defined for the SED Group '" << sedGroup << "' at redshift "
                              << z << ". Please check your configuration.";
}

const std::vector<std::pair<LuminosityFunctionValidityDomain, std::unique_ptr<MathUtils::Function>>>&
LuminosityFunctionSet::getFunctions() const {
