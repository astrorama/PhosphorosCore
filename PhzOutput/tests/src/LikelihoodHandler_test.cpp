...
class MockSampler {
public:
  MockSampler(const std::map<std::string, Euclid::PhzDataModel::PhotometryGrid>&) {}
  ...
  std::vector<Euclid::PhzOutput::GridSample>
  drawSample(std::size_t sample_number, const std::map<std::string, double>&,
...
BOOST_FIXTURE_TEST_CASE(test_PosteriorGridZ, LikelihoodFixture) {
  ...
  while (true) {
    ...
    LikelihoodHandler<Euclid::PhzDataModel::RegionResultType::POSTERIOR_LOG_GRID, MockSampler> likelihoodHandler(
        output_path.path(), param_config, model_grid_map, true, 10, 2);
...
//-----------------------------------------------------------------------------BOOST_FIXTURE_TEST_CASE(test_PosteriorGridZ,
// LikelihoodFixture) {
