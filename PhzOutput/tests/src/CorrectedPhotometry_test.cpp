...
BOOST_FIXTURE_TEST_CASE(test_getColumnInfoList, CorrectedPhotometry_fixture) {
	...
	// When
	auto corrected_photo_handler = PhzOutput::ColumnHandlers::CorrectedPhotometry(
			colInfo,
			filter_mapping,
			false,
			false,
...
BOOST_FIXTURE_TEST_CASE(test_getColumnInfoListError, CorrectedPhotometry_fixture) {
	...
	BOOST_CHECK_THROW(PhzOutput::ColumnHandlers::CorrectedPhotometry(
			colInfo,
			filter_mapping_err_f,
			false,
			false,
			false,
			1.0,
...
BOOST_FIXTURE_TEST_CASE(test_computeCorrectionFactorForModel, CorrectedPhotometry_fixture) {
	...
	// When
    auto corrected_photo_handler_2 = PhzOutput::ColumnHandlers::CorrectedPhotometry(
			colInfo,
			filter_mapping,
			false,
	...
	//Then
	BOOST_CHECK_EQUAL(correction.size(), 2);
	BOOST_CHECK_CLOSE(correction[0], 1.0/132, 0.001);  // correction factor = 1/(1.0 + shifts²*coef_f + shifts*coef_e) = 1/(1.0 +10²*1.1+10*2.1) = 1/132
	BOOST_CHECK_CLOSE(correction[1], 1.0/1323, 0.001);  // 1/(1+20²*3.1 +20*4.1) = 1/1323

	// When
	auto corrected_photo_handler_3 = PhzOutput::ColumnHandlers::CorrectedPhotometry(
			colInfo,
			filter_mapping,
			false,
			false,            //  no filter correction
	...
	// When
	auto corrected_photo_handler_4 = PhzOutput::ColumnHandlers::CorrectedPhotometry(
			colInfo,
			filter_mapping,
			false,
			true,            //  filter correction
