...
PhzCatalog::PhzCatalog(boost::filesystem::path out_file, Format format,
                       std::vector<std::shared_ptr<ColumnHandler>> handler_list, std::vector<std::string> comments,
                       uint flush_chunk_size)
    : m_out_file{out_file}, m_handler_list{std::move(handler_list)}, m_flush_chunk_size(flush_chunk_size) {
