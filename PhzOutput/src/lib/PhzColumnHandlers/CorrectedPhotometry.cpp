...
CorrectedPhotometry::CorrectedPhotometry(
    const Table::ColumnInfo& column_info, const Configuration::PhotometricBandMappingConfig::MappingMap& mapping,
  ...
  logger.info() << "Output corrected photometry = " << m_do_marginalize << " with filter_shift :" << m_correct_filter
...
std::vector<double> CorrectedPhotometry::sumVector(double m1, std::vector<double> v1, double m2,
...
CorrectedPhotometry::convertResults(const SourceCatalog::Source&       source,
  ...
  if (m_do_marginalize) {
    ...
    PhzDataModel::PhotometryGrid::const_iterator best_fit_model =
        sourceResult.get<PhzDataModel::SourceResultType::BEST_MODEL_ITERATOR>();
