...
/**
 * @file src/lib/PhzColumnHandlers/BestLikelihoodModel.cpp
 * @date 19/05/17
 * @author dubathf
 */
...
std::vector<Table::ColumnInfo::info_type> BestLikelihoodModel::getColumnInfoList() const {

  return std::vector<Table::ColumnInfo::info_type> {
    Table::ColumnInfo::info_type("LIKELIHOOD-SED", typeid(std::string)),
    Table::ColumnInfo::info_type("LIKELIHOOD-SED-Index", typeid(int64_t)),
    Table::ColumnInfo::info_type("LIKELIHOOD-ReddeningCurve", typeid(std::string)),
    Table::ColumnInfo::info_type("LIKELIHOOD-E(B-V)", typeid(double)),
    Table::ColumnInfo::info_type("LIKELIHOOD-Z", typeid(double)),
    Table::ColumnInfo::info_type("LIKELIHOOD-Scale", typeid(double))
  };
}

std::vector<Table::Row::cell_type> BestLikelihoodModel::convertResults(
                      const SourceCatalog::Source&,
                      const PhzDataModel::SourceResults& results) const {

  auto& best_model = results.get<PhzDataModel::SourceResultType::BEST_LIKELIHOOD_MODEL_ITERATOR>();
