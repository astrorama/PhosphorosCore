...
PhotometricCorrectionCalculator::PhotometricCorrectionCalculator(
    FindBestFitModelsFunction find_best_fit_models, CalculateScaleFactorsMapFunction calculate_scale_factors_map,
...
PhzDataModel::PhotometricCorrectionMap PhotometricCorrectionCalculator::operator()(
    const SourceCatalog::Catalog& catalog, const std::map<std::string, PhzDataModel::PhotometryGrid>& model_grid_map,
    StopCriteriaFunction stop_criteria_func, SelectorFunction selector, ProgressListener progress_listener) {
  ...
  while (!stop_criteria_func(phot_corr)) {
    ...
    auto best_fit_model_map = m_find_best_fit_models(catalog.begin(), catalog.end(), model_grid_map, phot_corr);
    auto scale_factor_map   = m_calculate_scale_factors_map(catalog.begin(), catalog.end(), best_fit_model_map);
    phot_corr = m_calculate_photometric_correction(catalog.begin(), catalog.end(), scale_factor_map, best_fit_model_map,
                                                   selector);
