...
BOOST_AUTO_TEST_CASE(maxIterationsStop) {
  ...
  // When
  int counter = 0;
  while (!checker({{XYDataset::QualifiedName("Filter1"), counter}, {XYDataset::QualifiedName("Filter2"), counter}})) {
    ++counter;
  }
...
BOOST_AUTO_TEST_CASE(toleranceStop) {
  ...
  while (!stop) {
    ++counter;
    PhzDataModel::PhotometricCorrectionMap map{{XYDataset::QualifiedName("Filter1"), 1. / counter},
