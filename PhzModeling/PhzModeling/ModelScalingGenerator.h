...
/*
 * PhzModeling/ModelScalingGenerator.h
 *
 *  Created on: 2023/04/25
 *      Author: fdubath
 */

#ifndef PHZMODELING_MODELSCALINGGENERATOR_H_
#define PHZMODELING_MODELSCALINGGENERATOR_H_
...
/**
 * @class ModelScalingGenerator
 * @brief Provides the Scaling for the current index of the Model
...
public:
   ...
   */
  ModelScalingGenerator(const ModelScalingGenerator& other);
   ...
   *
   * @param other
   * A ModelScalingGenerator: the index has to be copied from.
   *
   */
  ModelScalingGenerator& operator=(const ModelScalingGenerator& other);
   ...
   *
   */
  ModelScalingGenerator& operator=(size_t other);

  /**
   * @brief Increment operator. Increase the current index and reset
   * the current model.
   */
  ModelScalingGenerator& operator++();
   ...
   */
  ModelScalingGenerator& operator+=(int n);
   ...
   * the provided ModelScalingGenerator and reset the current reddened SED.
   *
   * @param other
   * A ModelScalingGenerator from which index value the current index will
   * be decremented.
   */
  int operator-(const ModelScalingGenerator& other) const;
   ...
   *
   * @param other
   * A ModelScalingGenerator from which the the current index will be
   * compared to.
   */
  bool operator==(const ModelScalingGenerator& other) const;
   ...
   *
   * @param other
   * A ModelScalingGenerator the current index will be compared to.
   */
  bool operator>(const ModelScalingGenerator& other) const;
   ...
   * @details
   * Compute the scaling of the SED for the current index and return a reference on it
   *
   * @return
   * A double representing the scaling for the current index
   */
  double& operator*();
...
};  // End of ModelScalingGenerator class

}  // end of namespace PhzModeling
}  // end of namespace Euclid

#endif /* PHZMODELING_MODELSCALINGGENERATOR_H_ */
