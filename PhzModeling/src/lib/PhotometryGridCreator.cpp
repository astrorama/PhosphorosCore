...
template <typename NameIter>
std::map<XYDataset::QualifiedName, XYDataset::XYDataset> buildFilterMap(XYDataset::XYDatasetProvider& provider,
                                                                        NameIter begin, NameIter end) {
  std::map<XYDataset::QualifiedName, XYDataset::XYDataset> result{};
...
convertToFunction(const std::map<XYDataset::QualifiedName, XYDataset::XYDataset>& dataset_map) {
  ...
  for (auto& pair : dataset_map) {
    try {
      ...
      throw Elements::Exception() << "Failed to interpolate " << pair.first.qualifiedName() << ": " << ex.what();
    }
  }
  return result;
}

PhotometryGridCreator::PhotometryGridCreator(std::shared_ptr<XYDataset::XYDatasetProvider> sed_provider,
