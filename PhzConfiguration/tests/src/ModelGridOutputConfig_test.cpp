...
struct ModelGridOutputConfig_fixture : public ConfigManager_fixture {
  ...
  std::shared_ptr<std::vector<std::string>> filter_2 =
      std::shared_ptr<std::vector<std::string>>(new std::vector<std::string>{"filter1", "filter2", "filter3"});
  std::shared_ptr<std::vector<std::string>> filter_3 =
      std::shared_ptr<std::vector<std::string>>(new std::vector<std::string>{"filter1", "filter3"});
  ...
  ModelGridOutputConfig_fixture() {
    ...
    auto path_filter_1 =
        (Elements::getAuxiliaryPath("Phosphoros/AuxiliaryData/Filters") / std::string(filter_lum + ".txt")).string();
...
struct archive {
  typedef I          iarchive;
  static std::string getFormatOption() {
    return OptionStr;
  };
};
