...
struct LuminosityFunctionConfig_fixture :public ConfigManager_fixture{
  ...
  LuminosityFunctionConfig_fixture() {
    ...
    options_map["luminosity-filter"].value() = boost::any(std::string {
        "Test/Filter" });
  ...
  void addGroup(double min, double max, std::string group, std::string f_name) {
    options_map["luminosity-function-sed-group-" + f_name].value() = boost::any(
        group);
    options_map["luminosity-function-min-z-" + f_name].value() = boost::any(
        min);
    options_map["luminosity-function-max-z-" + f_name].value() = boost::any(
        max);
  }

  void addCustomCurve(std::string curve_name, std::string f_name) {
    options_map["luminosity-function-curve-" + f_name].value() = boost::any(
        curve_name);
  }
  void addSchechter(bool in_mag, double alpha, double mo, double lo,
      double phio, std::string f_name) {
    options_map["luminosity-function-schechter-alpha-" + f_name].value() =
        boost::any(alpha);
    if (in_mag) {
      options_map["luminosity-function-schechter-m0-" + f_name].value() =
          boost::any(mo);
    } else {
      options_map["luminosity-function-schechter-l0-" + f_name].value() =
          boost::any(lo);
    }
    options_map["luminosity-function-schechter-phi0-" + f_name].value() =
        boost::any(phio);
  }
