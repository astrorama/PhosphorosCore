...
struct PhotometricCorrectionConfig_fixture : public ConfigManager_fixture {
  ...
  PhotometricCorrectionConfig_fixture() {
    ...
    options_map["filter-mapping-file"].value() =
        boost::any((temp_dir.path() / catalog_type / filter_mapping_file).string());
...
BOOST_FIXTURE_TEST_CASE(nominal_test, PhotometricCorrectionConfig_fixture) {
  ...
  auto& photometric_coeffs =
      config_manager.getConfiguration<PhotometricCorrectionConfig>().getPhotometricCorrectionMap();
