...
template<int I>
PhzLikelihood::AxisWeightPrior<I> createPrior(const std::string& axis_name,
      ...
      for (auto& pair : regions) {
        for (auto& name : std::get<I>(pair.second)) {
          if (weights.find(name) == weights.end()) {
            throw Elements::Exception() << "Missing weight for " << name.qualifiedName()
  ...
  throw Elements::Exception() << "Couldn't find weights " << table_name
          << " in directory " << dir.string();
}
...
void AxisWeightPriorConfig::initialize(const UserValues& args) {
  ...
  for (auto& axis_name : findWildcardOptions({AXIS_WEIGHT_PRIOR}, args)) {
    for (auto& table_name : args.at(AXIS_WEIGHT_PRIOR+"-"+axis_name).as<std::vector<std::string>>()) {
      if (axis_name == "sed") {
        getDependency<PriorConfig>().addPrior(createPrior<PhzDataModel::ModelParameter::SED>(
                                                      axis_name, table_name, aux_dir, regions));
      } else if (axis_name == "red-curve") {
        getDependency<PriorConfig>().addPrior(createPrior<PhzDataModel::ModelParameter::REDDENING_CURVE>(
                                                      axis_name, table_name, aux_dir,regions));
      }
    }
  }
