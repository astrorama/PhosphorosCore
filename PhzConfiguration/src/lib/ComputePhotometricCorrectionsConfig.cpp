...
ComputePhotometricCorrectionsConfig::ComputePhotometricCorrectionsConfig(long manager_id) : Configuration(manager_id) {
...
auto ComputePhotometricCorrectionsConfig::getProgramOptions() -> std::map<std::string, OptionDescriptionList> {
  return {{"Compute Photometric Corrections options",
           {{OUTPUT_PHOT_CORR_FILE.c_str(), boost::program_options::value<std::string>(),
            ...
            {PHOT_CORR_TOLERANCE.c_str(), boost::program_options::value<double>()->default_value(1E-3),
             ...
             "iteration stops"},
            {PHOT_CORR_SELECTION_METHOD.c_str(), boost::program_options::value<std::string>()->default_value("MEDIAN"),
             "The method used for selecting the photometric correction (MEDIAN, "
             "WEIGHTED_MEDIAN, MEAN, WEIGHTED_MEAN)"}}}};
}

void ComputePhotometricCorrectionsConfig::preInitialize(const UserValues& args) {
  auto method = args.at(PHOT_CORR_SELECTION_METHOD).as<std::string>();
  if (method != "MEDIAN" && method != "WEIGHTED_MEDIAN" && method != "MEAN" && method != "WEIGHTED_MEAN") {
    logger.error() << "Unknown photometric correction selection method " << method;
    throw Elements::Exception() << "Unknown photometric correction selection method " << method;
  }
}

static fs::path getOutputPathFromOptions(const std::map<std::string, po::variable_value>& args,
                                         const fs::path& intermediate_dir, const std::string& catalog_type) {
  fs::path result = (args.find(OUTPUT_PHOT_CORR_FILE) != args.end()) ? args.at(OUTPUT_PHOT_CORR_FILE).as<std::string>()
...
ComputePhotometricCorrectionsConfig::PhotCorrSelectorType initializePhotCorrSelector(const std::string& method) {
  if (method == "MEDIAN") {
  ...
  throw Elements::Exception() << "Unknown " << PHOT_CORR_SELECTION_METHOD << " : " << method;
}
...
void ComputePhotometricCorrectionsConfig::initialize(const UserValues& args) {
  ...
  auto  filename         = getOutputPathFromOptions(args, intermediate_dir, catalog_type).string();
  ...
  auto& grid_filter_names = getDependency<PhotometryGridConfig>().getPhotometryGridInfo().filter_names;
  std::vector<std::string> band_mapping_names{};
  for (auto& pair : getDependency<PhotometricBandMappingConfig>().getPhotometricBandMapping()) {
    band_mapping_names.emplace_back(pair.first);
  }
  checkGridPhotometriesMatch(grid_filter_names, band_mapping_names);

  // Initialize the output function
  m_output_function = [filename](const PhzDataModel::PhotometricCorrectionMap& pc_map) {
    auto          local_logger = Elements::Logging::getLogger("PhzOutput");
    std::ofstream out{filename};
    PhzDataModel::writePhotometricCorrectionMap(out, pc_map);
    local_logger.info() << "Created zero point corrections in file " << filename;
  };
  ...
  m_stop_criteria  = PhzPhotometricCorrection::DefaultStopCriteria(iter_no, tolerance);
...
const ComputePhotometricCorrectionsConfig::OutputFunction& ComputePhotometricCorrectionsConfig::getOutputFunction() {
...
auto ComputePhotometricCorrectionsConfig::getPhotometricCorrectionSelector() -> const PhotCorrSelectorType& {
  if (getCurrentState() < State::INITIALIZED) {
    throw Elements::Exception() << "getPhotometricCorrectionSelector() call on uninitialized "
                                << "ComputePhotometricCorrectionsConfig";
  }
  return m_phot_corr_selector;
}

const PhzPhotometricCorrection::PhotometricCorrectionCalculator::StopCriteriaFunction&
