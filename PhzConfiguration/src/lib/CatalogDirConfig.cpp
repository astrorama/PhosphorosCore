...
auto CatalogDirConfig::getProgramOptions() -> std::map<std::string, OptionDescriptionList> {
  return {{"Directory options",
           {{CATALOGS_DIR.c_str(), po::value<std::string>(), "The directory containing the catalog files"}}}};
}

static fs::path getCatalogsDirFromOptions(const std::map<std::string, po::variable_value>& args, const fs::path& root) {
