...
void LuminositySedGroupConfig::initialize(const UserValues& args) {
  ...
  for (auto& group_name : group_name_list) {
    ...
    boost::split(sed_names, sed_list, boost::is_any_of(","), boost::token_compress_on);
  ...
  std::set_difference(grid_seds.begin(), grid_seds.end(), all_group_seds.begin(), all_group_seds.end(),
  ...
  getDependency<MarginalizationConfig>().addMarginalizationCorrection(
      PhzDataModel::ModelParameter::SED, PhzLikelihood::GroupedAxisCorrection<PhzDataModel::ModelParameter::SED>{
...
const PhzDataModel::QualifiedNameGroupManager& LuminositySedGroupConfig::getLuminositySedGroupManager() {
  ...
  if (!m_is_enabled) {
    throw Elements::Exception()
        << "getLuminositySedGroupManager() call when Luminosity prior functionality is disabled";
  }
