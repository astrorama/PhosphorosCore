...
GalactiAbsorptionModelGridModifConfig::GalactiAbsorptionModelGridModifConfig(long manager_id) : Configuration(manager_id) {
...
bool areGridsCompatible(const PhzDataModel::PhotometryGridInfo& first_grid, const PhzDataModel::PhotometryGridInfo second_grid) {
  ...
  for (auto& iter : first_grid.region_axes_map) {
    ...
    if (second_axes_it == second_grid.region_axes_map.end()) {
      logger.error()
              << "The region named '" << iter.first << "' is not found in the second grid.";
      return false;
    }
...
void GalactiAbsorptionModelGridModifConfig::postInitialize(const UserValues& args) {
  if (getDependency<DustColumnDensityColumnConfig>().isGalacticCorrectionEnabled()) {
