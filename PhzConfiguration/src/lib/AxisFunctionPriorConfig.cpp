...
template<int I>
PhzLikelihood::AxisFunctionPrior<I> createPrior(const std::string& axis_name,
...
void AxisFunctionPriorConfig::initialize(const UserValues& args) {
  ...
  for (auto& axis_name : findWildcardOptions({AXIS_FUNCTION_PRIOR}, args)) {
    for (auto& dataset_name : args.at(AXIS_FUNCTION_PRIOR+"-"+axis_name).as<std::vector<std::string>>()) {
      if (axis_name == "z") {
        getDependency<PriorConfig>().addPrior(createPrior<PhzDataModel::ModelParameter::Z>(
                                                      axis_name, dataset_name, *provider_ptr));
      } else if (axis_name == "ebv") {
        getDependency<PriorConfig>().addPrior(createPrior<PhzDataModel::ModelParameter::EBV>(
                                                      axis_name, dataset_name, *provider_ptr));
      }
    }
  }
