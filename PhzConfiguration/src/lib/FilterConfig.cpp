...
auto FilterConfig::getProgramOptions() -> std::map<std::string, OptionDescriptionList> {
  return {{"Filter options",
           {{FILTER_GROUP.c_str(), po::value<std::vector<std::string>>(),
            ...
            {FILTER_EXCLUDE.c_str(), po::value<std::vector<std::string>>(), "Exclude a single filter name"},
            {FILTER_NAME.c_str(), po::value<std::vector<std::string>>(), "Add a single filter name"}}}};
}
