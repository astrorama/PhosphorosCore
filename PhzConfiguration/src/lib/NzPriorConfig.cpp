...
void NzPriorConfig::preInitialize(const UserValues& args) {
  if (args.at(NZ_PRIOR).as<std::string>() != "NO" && args.at(NZ_PRIOR).as<std::string>() != "YES") {
    throw Elements::Exception() << "Invalid " + NZ_PRIOR + " value: " << args.at(NZ_PRIOR).as<std::string>()
...
void NzPriorConfig::initialize(const UserValues& args) {
  if (args.at(NZ_PRIOR).as<std::string>() == "YES") {
    ...
    auto param = PhzNzPrior::NzPriorParam(z0_t1, km_t1, alpha_t1, k_t1, f_t1, cst_t1, z0_t2, km_t2, alpha_t2, k_t2,
                                          f_t2, cst_t2, z0_t3, km_t3, alpha_t3, cst_t3);
    ...
    // get the sed list
    const std::map<std::string, PhzDataModel::PhotometryGrid>& grids =
        getDependency<PhotometryGridConfig>().getPhotometryGrid();
