...
static void
checkForMissingCoordinates(const std::map<std::string, std::vector<XYDataset::QualifiedName>>& sed_map,
  ...
  std::set_symmetric_difference(sed_region_names.begin(), sed_region_names.end(), red_curve_region_names.begin(),
                                red_curve_region_names.end(), std::back_inserter(regions_with_missing_coordinates));
  std::set_symmetric_difference(sed_region_names.begin(), sed_region_names.end(), ebv_region_names.begin(),
                                ebv_region_names.end(), std::back_inserter(regions_with_missing_coordinates));
  std::set_symmetric_difference(sed_region_names.begin(), sed_region_names.end(), z_region_names.begin(),
                                z_region_names.end(), std::back_inserter(regions_with_missing_coordinates));
...
static bool overlapingValues(const std::vector<double>& first, const std::vector<double>& second) {
        ...
        if (overlapingNames(red_curve_map.at(sed_first->first), red_curve_map.at(sed_second->first))) {
          if (overlapingValues(ebv_map.at(sed_first->first), ebv_map.at(sed_second->first))) {
            if (overlapingValues(z_map.at(sed_first->first), z_map.at(sed_second->first))) {
              throw Elements::Exception() << "Parameter space regions \"" << sed_first->first << "\" and \""
                                          << sed_second->first << "\" overlap";
            }
          }
        }
      }
...
void ParameterSpaceConfig::initialize(const UserValues&) {
  ...
  for (auto& sed_pair : sed_map) {
    ...
    result.emplace(std::make_pair(
        name, PhzDataModel::createAxesTuple(z_map[name], ebv_map[name], red_curve_map[name], sed_pair.second)));
  }
