...
ErrorAdjustmentConfig::ErrorAdjustmentConfig(long manager_id) : Configuration(manager_id) {
...
void ErrorAdjustmentConfig::initialize(const UserValues& args) {
  ...
  if (flag == "YES") {
    ...
    if (!fs::exists(param_file)) {
      ...
      throw Elements::Exception() << "Photometric Error adjustment file (" << ERROR_ADJUSTMENT_PARAM_FILE
                                  << ") does not exist : " << param_file;
    }
