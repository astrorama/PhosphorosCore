...
void ComputeModelSedConfig::initialize(const UserValues& args) {
  ...
  m_grid_axes.reset(new PhzDataModel::ModelAxesTuple{PhzDataModel::createAxesTuple({z}, {ebv}, {red_curve}, {sed})});
  ...
  m_red_curve_map.emplace(
      std::make_pair(red_curve, MathUtils::interpolate(*red_curve_data, MathUtils::InterpolationType::LINEAR)));
}
...
const std::map<XYDataset::QualifiedName, std::unique_ptr<MathUtils::Function>>&
ComputeModelSedConfig::getReddeningCurveMap() {
  if (getCurrentState() < State::INITIALIZED) {
