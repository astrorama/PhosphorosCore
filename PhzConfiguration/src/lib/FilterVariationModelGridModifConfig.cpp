...
static Elements::Logging logger = Elements::Logging::getLogger("FilterVariationModelGridModifConfig");

FilterVariationModelGridModifConfig::FilterVariationModelGridModifConfig(long manager_id) : Configuration(manager_id) {
...
void FilterVariationModelGridModifConfig::postInitialize(const UserValues& /* args */) {
  if (getDependency<ObservationConditionColumnConfig>().isFilterVariationEnabled()) {
    ...
    const std::map<std::string, PhzDataModel::PhotometryGrid>& coef_grid =
        getDependency<FilterVariationCoefficientGridConfig>().getFilterVariationCoefficientGrid();
    std::shared_ptr<PhzLikelihood::FilterShiftProcessModelGridFunctor> ptr{
        new PhzLikelihood::FilterShiftProcessModelGridFunctor(coef_grid)};
