...
auto ReddeningConfig::getProgramOptions() -> std::map<std::string, OptionDescriptionList> {
  return {{"Extinction options",
           {{REDDENING_CURVE_GROUP.c_str(), po::value<std::vector<std::string>>(),
            ...
            {REDDENING_CURVE_NAME.c_str(), po::value<std::vector<std::string>>(), "A single reddening curve name"},
            ...
            {EBV_RANGE.c_str(), po::value<std::vector<std::string>>(), "E(B-V) range: minimum maximum step"},
            {(EBV_RANGE + "-*").c_str(), po::value<std::vector<std::string>>(),
             ("The same as " + EBV_RANGE + " but for a specific parameter space region").c_str()},
            {EBV_VALUE.c_str(), po::value<std::vector<std::string>>(), "A single E(B-V) value"},
            {(EBV_VALUE + "-*").c_str(), po::value<std::vector<std::string>>(),
             ("The same as " + EBV_VALUE + " but for a specific parameter space region").c_str()}}}};
}

static std::vector<XYDataset::QualifiedName>
getRegionReddeningCurveList(const std::string& region_name, const std::map<std::string, po::variable_value>& options,
  ...
  if (options.count(EBV_RANGE + postfix) > 0) {
    ...
    for (auto& range : ranges_list) {
      ...
      if (!selected.empty() && *selected.rbegin() > min) {
        throw Elements::Exception() << "Overlapping range for " << (EBV_RANGE + postfix) << " option : " << min << ' '
                                    << max << ' ' << step;
      }
...
void ReddeningConfig::initialize(const UserValues& args) {
  ...
  auto region_name_list =
      findWildcardOptions({REDDENING_CURVE_NAME, REDDENING_CURVE_GROUP, REDDENING_CURVE_EXCLUDE}, args);
