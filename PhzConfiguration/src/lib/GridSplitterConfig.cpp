  ...
  if (m_splitting_dim == Euclid::PhzDataModel::ModelParameter::Z ||
    ...
    for (auto it_grid = grid_info.region_axes_map.begin(); it_grid != grid_info.region_axes_map.end(); ++it_grid) {
      ...
      for (auto axe_iter = axis.begin(); axe_iter != axis.end(); ++axe_iter) {
        std::string num_text = std::to_string(*(axe_iter));
        std::string rounded  = num_text.substr(0, num_text.find(".") + 6);
        all_values.insert(rounded);
      }
    }
    std::string previous = "";
    for (auto it_val = all_values.begin(); it_val != all_values.end(); ++it_val) {
      if (previous != "") {
        std::vector<std::string> vals{previous, *it_val};
        m_slice_def.emplace(std::make_pair(
            boost::filesystem::path(m_output_folder.string() + "/SubGrid_" + previous + "-" + *it_val + ".txt"), vals));
      }
      previous = *it_val;
    }
  } else {
    ...
    for (auto it_val = all_values.begin(); it_val != all_values.end(); ++it_val) {
      auto                     cleaned_name = std::regex_replace(it_val->qualifiedName(), std::regex("/"), "|");
      std::vector<std::string> vals{it_val->qualifiedName()};
      boost::filesystem::path  path{m_output_folder.string() + "/SubGrid_" + cleaned_name + ".txt"};
      m_slice_def.emplace(std::make_pair(path, vals));
    }
  }
}
