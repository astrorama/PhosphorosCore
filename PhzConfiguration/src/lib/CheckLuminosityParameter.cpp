...
bool CheckLuminosityParameter::checkSedGroupCompletness(
    const PhzDataModel::PhotometryGridInfo&        modelGridInfo,
  ...
  if (group_size != grid_seds.size()) {
    logger.info() << "SED lists have not the same size (SEDs in SED groups) " << group_size << " != (SEDs in the grid) "
                  << grid_seds.size();
    return false;
  }
