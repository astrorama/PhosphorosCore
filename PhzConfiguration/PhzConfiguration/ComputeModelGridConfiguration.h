...
public:
   ...
   * @details
   * The option is:
   *  - \b output-model-grid : string, output filename for storing the grid data
   *  The constructor also makes sure we are able to write a file to the location
   *  defined by \b output-model-grid
   * @param options
   * A map containing the options and their values.
   * @throw Element::Exception
   * - IO error, can not write any file there(to the location defined by \b output-model-grid)
   *
   */
